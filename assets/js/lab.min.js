...
function getTextReason(selected_reason){
    let reasons = {
        "Repeat / Missing": 9144,
        "Targeted": 3280,
        "Confirmatory": 1345,
...
function createLabOrders(encounter) {
    for (var test_name in selected_tests){
        let test_reason = getTextReason(selected_tests[test_name]);
...
function updateOrderTable(orders) {
    ...
    for (var x = 0; x < orders.length; x++) {
        ...
        for(let i = 0; i <  tests.length; i++){
            let order_result = (tests[i].result == null ? "" : tests[i].result.value);
            let result_date = (tests[i].result == null ? "" : moment(tests[i].result.date).format("DD/MMM/YYYY"));
            console.log(tests[i].result)

            orders_table.row.add([tests[i].name, accession_number, test_status,
                date_ordered, order_result, result_date]).node().id = "lab-order-" + (i+1);
            orders_table.draw();
        }
