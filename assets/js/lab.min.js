...
function buildLabOrderDisplay() {
    ...
    var heads = ['Accession#','Test name', 'Specimen', 'Ordered', 'Result', 'Released', 'Given'];
...
function updateOrderTable(orders) {
    ...
    for (var x = 0; x < orders.length; x++) {
        ...
        if(ordersAccessionNumbers[accession_number] == null){
            ordersAccessionNumbers[accession_number] = [];
        }
        ordersAccessionNumbers[accession_number].push(orders[x]);
        ...
        for(let i = 0; i <  tests.length; i++){
            ...
            orders_table.row.add([addPrintLink(accession_number), tests[i].name, test_status,
                date_ordered, results_arr.join('<br />'), result_date, 
                `<span id="${accession_number}-given">N/A</span>`]).node().id = "lab-order-" + (i+1);
            orders_table.draw();
    ...
    try {
        labResultsGivenToClient(orders);
    } catch(e) {
        console.log(e);
    }
}

function addPrintLink(accession_number) {
    if(!extended_lab)
        return accession_number;

    return `<a href="javascript:void(0)" onclick="printLabOrder('${accession_number}')">${accession_number}</a>`;
}

function printLabOrder(accession_number){
    let orders = [];
    for(const order in ordersAccessionNumbers[accession_number]){
        orders.push(ordersAccessionNumbers[accession_number][order]);
    }
    printOrders(orders);
}
...
function addPrintBTN(orderID) {
  var span = document.createElement('span');
  var btn = document.createElement('button');
  btn.setAttribute('class','bootstrap-btns');
  btn.innerHTML = '<span>Reprint</span>';
  btn.setAttribute('onmousedown', `reprintOrder(${orderID})`);
  span.appendChild(btn);
  return span.innerHTML;
}
function reprintOrder(order_ids) {
 window.location.href = `/views/print/order.html?order_ids=${order_ids}&next_page=labs`;
}
