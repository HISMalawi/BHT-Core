...
if(sessionStorage.lab_is_set == 'true') {
    reset_lab = 'done';
  }
...
function activateDataTable() {
    orders_table = jQuery('#lab-orders').DataTable({
        ...
        columnDefs: [
            ...
            {"className": "date-col", type: 'date', "targets": 5}
          ], 
        paging: false,
        scroller: {
...
function changeCancelBTN() {
  ...
  if (sessionStorage.lab_is_set == 'true') {
    sessionStorage.lab_is_set =false;
    c.setAttribute('onmousedown','closeOrdersPopupModal(); reset_lab_and_radio_setting()');
    return;
  }
  c.setAttribute('onmousedown', 'changeCancelBG();' + cancelAttributes);
}

function reset_lab_and_radio_setting() {
...
function buildLabOrderDisplay() {
    ...
    var heads = ['Accession#','Test name', 'Specimen', 'Ordered', 'Result', 'Released'];
...
function addFinishOrderBTN() {
    var root = document.getElementById('buttons');
    var finishBTN = document.createElement('button');
    finishBTN.setAttribute('onmousedown', 'redirectOnFinishOrder();');
    finishBTN.setAttribute('id', 'finishBTN');
    finishBTN.innerHTML = '<span>Finish</span>';
    finishBTN.setAttribute('class', 'button green navButton');
    root.appendChild(finishBTN);
}

function redirectOnFinishOrder() {
    if(sessionStorage.orderFlowStatus == 'true') {
        if(sessionStorage.radiology_is_set == 'true') {
        // sessionStorage.radiology_order_done = true;
        // sessionStorage.lab_is_set = false;
        // sessionStorage.lab_order_done = true;
        ...
        if(reset_lab == 'done') {
        //sessionStorage.lab_is_set = false;
        //sessionStorage.lab_order_done = true; //problem with this one
        sessionStorage.orderFlowStatus = false;
        window.location.href = '/apps/OPD/views/encounters/presenting_complaints.html';
        //views/encounters/presenting_complaints.html
        //closeOrdersPopupModal();
        // nextEncounter(sessionStorage.patientID, sessionStorage.programID);
        // return;
        // history.go(-1);
...
function pressOrder() {
    ...
    if(parseInt(sessionStorage.programID) == 14)
    {
        ...
        search_content.setAttribute('style','margin-bottom:5px');
        orderBox.appendChild(search_content);
...
function search_results()
{
    ...
    for(let i = 0; i < search_obs.length; i++){
        if(!search_obs[i].name.toLowerCase().match(search_value.toLowerCase())) 
            document.getElementById(search_obs[i].concept_id).style = 'display:none';
        else
        document.getElementById(search_obs[i].concept_id).style = 'display:';
    }
}
    ...
    for (var x = 0; x < orders.length; x++) {
        ...
        for(let i = 0; i <  tests.length; i++){
            ...
            orders_table.row.add([accession_number, tests[i].name, test_status,
                date_ordered, results_arr.join('<br />'), result_date ]).node().id = "lab-order-" + (i+1);
            orders_table.draw();
...
function addViewResultsBTN() {
    var span = document.createElement('span');
    var btn = document.createElement('button');
    btn.setAttribute('class','bootstrap-btns navButton red');
    btn.setAttribute('style','text-align: left; width: 150px; text-align: center;');
...
function buildTestTable(unsorted_tests){
    ...
    for(let i = 0; i < tests.length; i++){
            ...
            tableHTML += `<tr id="${tests[i].concept_id}">
...
function createExOrders(encounter) {
    ...
    tests_for_opd_lims = selected_tests_specimen_reason;
    postOrders({orders: lab_orders_data});

    //create_lab_order(,surname,user_id,address,gender,dob,dob_estimated,external_patient_number,specimen_type,requested_by,test_type)
...
function getPatientDemo() {
    ...
    req.onreadystatechange = function () {
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            try {
                patient_demo = {
                    age : results.person.birthdate,
                    gender : results.person.gender,
                    addressl1 : results.person.addresses[0].city_village,
                    addressl2 : results.person.addresses[0].address2,
                    phone_number : results.person.person_attributes[1].value
...
// for OPD LImS Integeration
function limsPatientObj(orders) {

var lims_push_data = [];
var dob = sessionStorage.patientDOB.split('/');
dob = dob[0] +'-'+ dob[1] +'-'+ dob[2];

for (key in tests_for_opd_lims) {

    try {
        lims_push_data.push({
            'firstname' : sessionStorage.given_name,
            'surname' : sessionStorage.family_name,
            'user_id' : sessionStorage.userID,
            'address' : patient_demo.addressl1,
            'gender' : patient_demo.gender,
            'dob' : dob,
            'phone_number' : patient_demo.phone_number,
            'dob_estimated' : '',
            'external_patient_number' : '',
            'specimen_type' : tests_for_opd_lims[key].specimen,
            'requested_by' : sessionStorage.username,
            'test_type' : key,
            'tracking_number' : orders[0].accession_number
         });
    }
...
function printOrders(orders){
    ...
    if (sessionStorage.programID == '') {
        obj = limsPatientObj(orders);
        submitParameters(obj, '/emr_lims_interface', '');
    }
...
function ordersPopupModal() {
    ...
    bottom.setAttribute('style','top: 93%; border-radius: 0px 0px 4px 4px;');
    ...
    nextButton.setAttribute('onmousedown','closeOrdersPopupModal2()');
  ...
  function closeOrdersPopupModal2() {
