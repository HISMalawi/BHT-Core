...
{
    buildTestTable(search_obs);
    console.log(search_obs);
}
...
function buildTestTable(unsorted_tests){
    ...
    for(let i = 0; i < tests.length; i++){
        if(tests[i].name.toLowerCase().match(search_value.toLowerCase())) 
        {
            tableHTML += `<tr>
    ...
    try {
   var left_test = document.getElementById('tests-left')
   left_test.appendChild(tableHTML + "</table>");
    } catch(e)
    {
